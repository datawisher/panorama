...
public class AuthFilter implements GlobalFilter, Ordered {
    ...
    private static final String ACESS_TOKEN_HEADER = "Authorization";
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        request = builder.header("X-USER-ID", userId).build();
    ...
    private String getUserIdByToken(String token) {
        String redisKey = String.join(":", ACCESS_TOKEN, token);
    ...
    private static String getToken(ServerHttpRequest request) {
        ...
        // 从请求头获取token
        String token = headers.getFirst(ACESS_TOKEN_HEADER);
        if (StringUtils.isBlank(token)) {
